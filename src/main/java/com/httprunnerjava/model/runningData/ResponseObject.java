...
public class ResponseObject implements Serializable, LogAble {
    ...
    public ResponseObject(Response srcRespObj) {
        ...
        try {
            // 将状态码添加到Allure报告中
            Allure.addAttachment("statusCode", String.valueOf(statusCode));
        } catch (Exception e) {
            log.error("添加statusCode到allure报告中失败，失败原因是：" + e.getMessage());
        }
        Optional.ofNullable(ResponseObject.getCurrentRespBody()).ifPresent(respBody -> {
            try {
                // 将body添加到Allure报告中
                Allure.addAttachment("responseBody", "application/json", respBody);
            } catch (Exception e) {
                log.error("添加responseBody到allure报告中失败，失败原因是：" + e.getMessage());
            }
            try {
